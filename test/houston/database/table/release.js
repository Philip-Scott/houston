/**
 * test/release/database/modal/release.js
 * Tests releases table for needed fields
 */
...
test('has necessary table columns', async (t) => {
  ...
  const one = await knex.schema.hasTable('releases')

  const two = await knex.schema.hasColumn('releases', 'project_key')
  const three = await knex.schema.hasColumn('releases', 'service_id')
  const four = await knex.schema.hasColumn('releases', 'tag')
